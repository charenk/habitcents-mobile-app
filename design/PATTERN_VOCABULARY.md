...
- Persistent positive communication is one pattern, `ui/InfoRibbon`: sage-light band, sprout, one caption line. Dismissible (X) for one-shot lines such as the first-run receipts; persistent (no X) for standing lines such as the quiet-day placeholder. Always inside a list section, below the content it comments on, below the content it comments on: under the log card it reads as a receipt, above the quick-log field it read as an instruction. The "never above an input" half of this rule was written when the input sat at the top of the pane; since ADR 0038 docked it at the bottom, everything is above it and only the first half is operative. A gentle first-run line resolves itself once the thing it waits for exists.
