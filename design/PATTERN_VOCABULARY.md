...
- The dashed-border card is the app's "add another" affordance inside content (add upcoming, the watch nudge). Reuse the card for any add-an-item entry in a list. The break-habit trigger left it on 2026-09-07 (Charen): it lives in a dock, and both Today docks are one composer shell, `components/today/DockCard.tsx`, a white pill with a snow pill field at a fixed 52pt and a round sage plus. Its label still reads the state where that matters: "Break your first habit" at zero habits, "Break another habit" after (ADR 0038). The dock is the one pill container in the app; the switchers above stay rounded rects.
