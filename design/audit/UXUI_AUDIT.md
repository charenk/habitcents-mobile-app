...
- **74 findings**: 2 P0, 18 P1, 32 P2, 22 P3. Two of these, UX-068 and UX-069, were caught during Phase A review rather than the original scan; two more, UX-070 and UX-071, were caught during Round 1/2 review. One more, UX-072, was coined in shipped code and a commit message on 2026-08-13 but never logged in this document until this pass, which restores it here, resolved, with the meaning it already carries in code and git history. Two more still, UX-073 and UX-074, were caught by the same device pass: running the app in the iOS simulator against seeded demo data, including a real CSV leak scan, rather than reading source. None were part of the initial 67.
- **67 resolved, 7 open.** Phase A (commit `65e725e`): 14 findings. Phase B (current working tree at the time): 11 findings, the palette/contrast decision. Round 1 (commits `618a361` Phase F / `0cd804a` Phases C+D, merged `5772001`): 13 findings, decision 2 (the scale) and decision 3's minimum (performance). Round 2 (current working tree): 27 findings, the accessibility-flow and touch-target batch, plus UX-015's premise correction via the new UX-070, plus the two new findings UX-070 and UX-071, both resolved on arrival. Device pass (commit `377c3b5`): 1 finding, UX-072, fixed the same day it was found and logged here for the first time. Both P0s are resolved; every P1 but two (UX-012, UX-073) is resolved. Every resolved finding below carries a Status line; nothing was deleted or renumbered.
- **7 open: five decisions or engineering items already tracked, plus two new findings routed to a separate bet, not this document's UX/UI remediation:**
  ...
  6. UX-073: the leak-scan biggest-leak card can overstate a candidate's true monthly rate by multiples (a fixture with a true $1,200/month rent rate rendered as "$4,000.00 a month"). Engine work, not a design pass; verification of the root cause is still owed.
  7. UX-074: the leak-scan results screen's own title renders under the status bar and dynamic island on notched devices. Pre-existing on `main`, not a regression from this branch.
...
- Resolved since the last run: essentially every accessibility-flow and touch-target finding (Round 2), the full type/spacing scale ratification and the performance architecture minimum (Round 1), plus the two ratified color-token AA failures and the category-detail honesty/anti-pattern bugs from the runs before that. UX-072, category detail's raw ISO dates in recent logs, was fixed the same day it was found (commit `377c3b5`) and is recorded resolved here for the first time. See decision 1-3's shipped notes and the changelog for what actually shipped, round by round. UX-073 and UX-074 are new, not resolved; see the 2026-08-13 changelog entry for how all three were found.
- Recommended next steps: close the 5 originally-open items above (one engineering task, four Charen decisions); route UX-073 and UX-074 to their own bet rather than a design pass; then run the owed on-device VoiceOver/TalkBack and Dynamic Type pass before beta (which is exactly the kind of pass that surfaced UX-072, UX-073, and UX-074). Nothing else in this report blocks it.
...
Status: **Open, routed to a separate bet, not this document's UX/UI remediation scope.** Record this honestly: the formula was traced and the implied window (27) derived arithmetically by solving the observed output backwards, but the pipeline itself was not instrumented or re-run to directly confirm `coveredDays` resolves to 27; that verification is still owed. Also record clearly: surfacing rent as a candidate at all is NOT the bug. The fixture's own `chequing-split-mixed-dates.expected.json` requires a "park" candidate with monthly cadence, so that part is intended behaviour. Related but separate, noted as an observation and not part of this finding's fix: rent renders with a "Break it" CTA rather than fixed-cost treatment; spec 5.4 says a Fixed-class habit gets no tracking CTA, only a tip card. Command: none, this is engine work.
...
Status: **Open, routed to a separate bet, not this document's UX/UI remediation scope.** Checked against `main` and confirmed pre-existing, not introduced by the remediation work on this branch: `git show main:app/leak-scan.tsx` and `git show main:components/leak-scan/ResultsScreen.tsx` both contain zero safe-area references, matching the current working tree. The only change this branch made to that screen's padding was horizontal 16 to the ratified 20 (`spacing.gutter`); vertical padding was left untouched.
