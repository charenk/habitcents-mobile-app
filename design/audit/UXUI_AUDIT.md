...
| | **Total** | **19 / 20** | **Very good: the code-level system is essentially complete. What remains is a device-verification pass and a single named item: UX-040, whose code and doc halves are both done and which is held open only pending Charen's Lane 2 sign-off. The same-day fix work closed the last three engineering/cleanup items (UX-012 scan yield, UX-041 swipe-to-dismiss, UX-075 token cleanup) in live code, on top of the device pass that closed UX-073/074 and UX-060. No new architecture remains** |
...
- **75 findings**: 2 P0, 18 P1, 32 P2, 23 P3. Two of these, UX-068 and UX-069, were caught during Phase A review rather than the original scan; two more, UX-070 and UX-071, were caught during Round 1/2 review. One more, UX-072, was coined in shipped code and a commit message on 2026-08-13 but never logged in this document until the 2026-08-13 pass, which restored it, resolved, with the meaning it already carries in code and git history. Two more still, UX-073 and UX-074, were caught by the device pass: running the app in the iOS simulator against seeded demo data, including a real CSV leak scan, rather than reading source. The newest, UX-075, was opened on 2026-08-22 for the leftover off-scale radius/spacing literal cleanup UX-018 flagged. None were part of the initial 67.
- **74 resolved, 1 open.** Phase A (commit `65e725e`): 14 findings. Phase B (current working tree at the time): 11 findings, the palette/contrast decision. Round 1 (commits `618a361` Phase F / `0cd804a` Phases C+D, merged `5772001`): 13 findings, decision 2 (the scale) and decision 3's minimum (performance). Round 2 (current working tree): 27 findings, the accessibility-flow and touch-target batch, plus UX-015's premise correction via the new UX-070, plus the two new findings UX-070 and UX-071, both resolved on arrival. Device pass (commit `377c3b5`): 1 finding, UX-072, fixed the same day it was found and logged. Device-pass close-out (verified 2026-08-22 against live code): UX-073, UX-074, and UX-060's remaining half, all now confirmed fixed in shipped code. Doc/comment close-out (2026-08-22): UX-065 documented and UX-040's documentation half ratified. Same-day fix work (commit `d5eebfd`, 2026-08-22): UX-012 (scan yields before the synchronous parse), UX-041 (real swipe-to-dismiss on the sheet), and UX-075 (off-scale radius/spacing literals converted to tokens) all shipped in code, closing the last three engineering/cleanup items. Both P0s are resolved; every P1 is now resolved. Every resolved finding below carries a Status line; nothing was deleted or renumbered.
- **1 open: a single item held open pending Charen's Lane 2 sign-off, no engineering left:**
  1. UX-040 (resolved-pending-verification): both halves are now done. ExpenseSheet already matched its Money-sheet siblings in code, and `PATTERN_VOCABULARY.md` now ratifies the serif sheetTitle head as the pattern (2026-08-22). Held in the open ledger only until Charen's Lane 2 sign-off, since ratifying a user-facing convention routes to a person. There is no code or cleanup work outstanding on it.
- **Now resolved this pass (2026-08-22, commit `d5eebfd`), moved out of the open list:** UX-012 (the scan pipeline yields via `InteractionManager.runAfterInteractions` before the synchronous `runScan`, so the "Reading your files" spinner paints and VoiceOver settles before the parse; regression test at `__tests__/leakScanIntakeYield.test.tsx`; no chunking, outputs unchanged), UX-041 (real swipe-to-dismiss on `components/ui/Sheet.tsx` via `PanResponder` driving the single existing translate driver, a 25%-of-height or downward-velocity-over-0.5 dismiss threshold, reduced-motion fallback, one-driver-per-node crash rule preserved; 75 sheet tests pass), and UX-075 (all `borderRadius: 3` converted to `radii.micro` and off-scale 3/5/7/22 spacing literals to `spacing` tokens across 17 files, TierBadge discs to `radii.pill`, off-scale grep count now zero). All three verified against live code.
- **Resolved just before this pass (2026-08-22, device-pass close-out):** UX-060 (LoggedTodayList's JS `.toUpperCase()` replaced by `textTransform: 'uppercase'` in the stylesheet, both halves now closed), UX-065 (AuroraBackground's borrowed hues now documented in a code comment as a sanctioned decorative borrow), UX-073 (the biggest-leak monthly-rate divisor is now the calendar span, `rankedLeaksWindowDays = Math.max(result.coverage?.spanDays ?? 0, 1)`, fixing the multiples overstatement), and UX-074 (the results header now applies `insets.top + 16` via `useSafeAreaInsets`, clearing the notch). All four verified against live code, not inferred.
- **Other open decisions, not tied to a single finding id:** the "stop breaking" color contradiction is resolved at the documentation level (2026-08-22): `constants/theme.ts`'s coral comment now states that "stop breaking" is intentionally rendered in muted slate and that coral is reserved for hard-destructive actions (delete, undo import), matching the shipped color; no shipped color was flipped (see UX-039's status). `app/onboarding/welcome.tsx` is still knowingly off ADR 0022 behind a ratify-or-revert comment; only its dead code was cleared (see UX-061's status and UX-017's two by-design fontSize exceptions). Note: UX-061's splash-hero premise is now obsolete anyway, since `welcome.tsx` was subsequently rebuilt as the ADR-0026 carousel; the splash hero it described is no longer the live screen.
- Resolved since the last run: essentially every accessibility-flow and touch-target finding (Round 2), the full type/spacing scale ratification and the performance architecture minimum (Round 1), plus the two ratified color-token AA failures and the category-detail honesty/anti-pattern bugs from the runs before that. UX-072, category detail's raw ISO dates in recent logs, was fixed the same day it was found (commit `377c3b5`). See decision 1-3's shipped notes and the changelog for what actually shipped, round by round. As of 2026-08-22 the device-pass findings UX-073 and UX-074 are both resolved in live code, alongside UX-060; see the 2026-08-22 changelog entry.
- Recommended next steps: the last engineering and cleanup items (UX-012 scan yield, UX-041 swipe-to-dismiss, UX-075 token cleanup) all shipped in the same-day fix work (commit `d5eebfd`), so what remains is a Lane 2 sign-off from Charen on the UX-040 pattern-vocabulary ratification, and then the owed on-device VoiceOver/TalkBack and Dynamic Type pass before beta (which is exactly the kind of pass that surfaced UX-072, UX-073, and UX-074). Nothing else in this report blocks it.
...
- **UX-041, sheet grab handle. RESOLVED, same-day fix work (commit `d5eebfd`).** The decision landed in favour of adding the gesture: `components/ui/Sheet.tsx` now implements real swipe-to-dismiss via `PanResponder` driving the single existing translate driver (25%-of-height or downward-velocity-over-0.5 dismiss threshold, reduced-motion fallback, one-driver-per-node crash rule preserved). No longer an open decision; recorded here so the prior open item is not silently dropped.
- **UX-040, Money sheet header pattern (resolved-pending-verification).** The serif `sheetTitle` treatment already won in code (ExpenseSheet was moved onto it in Round 2), and `PATTERN_VOCABULARY.md` now ratifies it under Surfaces (2026-08-22). Both halves done; awaiting Charen's Lane 2 sign-off on the doc change before final close. This is the one remaining item in the open ledger.
- **The "stop breaking" color contradiction (resolved at the documentation level, 2026-08-22).** The contradiction was between the token comment and the shipped color; it is resolved in favour of the shipped color. `constants/theme.ts`'s coral comment now states that "stop breaking" is intentionally rendered in muted slate (stopping a habit is a routine choice, not a data loss) and that coral is reserved for hard-destructive actions (delete, undo import). No shipped color was changed; the comment now matches the product (see UX-039).
...
Status: **Resolved**, same-day fix work (commit `d5eebfd`). `components/leak-scan/useLeakScanIntake.ts` now imports `InteractionManager` and yields via `await new Promise(resolve => InteractionManager.runAfterInteractions(() => resolve()))` immediately before the synchronous `runScan(...)` in `runWithRules`, so the "scanning" stage paints and VoiceOver settles before the blocking parse; the correction re-run path in `components/leak-scan/ResultsScreen.tsx` got the same yield. Regression test at `__tests__/leakScanIntakeYield.test.tsx`. No chunking was added and outputs are unchanged; the freeze is gone because the spinner now paints before the parse, not because the parse got faster.
...
**UX-073 · P1 · SYSTEM-GAP · honesty: the biggest-leak card can overstate a candidate's true monthly rate by multiples.**
Location: `utils/leakScanBridge.ts:71` (`totalMonthlySpend = Math.round((candidate.totalCents / windowDays) * 30)`, with `windowDays = Math.max(coveredDays, 1)` at `:68`), rendered by `components/leak-scan/BiggestLeakCard.tsx:49-56` via `strings.habitLogging.leakEvidenceReliable`. Not in the original 71; found by running the app in the iOS simulator against seeded demo data and a real CSV leak scan, which is why the original audit, which read code rather than running it, missed it.
Impact: the repo's own eval fixture `__tests__/leakScanEval/fixtures/chequing-split-mixed-dates.csv` contains exactly three rent rows of $1200.00, dated 2026-04-01, 2026-05-01, and 2026-06-01: a true rate of $1,200 a month. The biggest-leak card rendered "Park costs you about $4,000.00 a month. You bought it 3 times in the last 3 months." while the same screen's evidence window read "APR 1 TO JUN 26," an 86-day span. Solving backwards, `3600 / windowDays * 30 = 4000` implies `windowDays = 27`, so the divisor appears to be counting days that carried a transaction rather than the calendar span the evidence window itself states, overstating the rate by roughly 3.3x. This is the app's single most prominent sentence on the results screen stating a fabricated figure, against the ratified rule "never invent statistics" and "no invented totals": the honesty layer failing in the one place the product is loudest.
Fix: not diagnosed to a code fix here; this is scan-pipeline coverage/window accounting, engine work, not a design or UI change.
Status: **Resolved**, device-pass fix (verified 2026-08-22 against live code). The monthly-rate divisor is now the calendar span, never the count of transacted days: `components/leak-scan/ResultsScreen.tsx:250` computes `rankedLeaksWindowDays = Math.max(result.coverage?.spanDays ?? 0, 1)` and the in-code comment cites this finding ("Calendar span, never the count of transacted days (UX-073)"). Feeding the elapsed calendar span into `monthlyCostCents` (`:257`) fixes the overstatement the fixture exposed: a true $1,200/month rent rate no longer renders as multiples of itself, because the rate is now divided by the same 86-day span the evidence window states rather than the ~27 transacted days. Also recorded, still true and unchanged: surfacing rent as a candidate at all was never the bug (the fixture's own `chequing-split-mixed-dates.expected.json` requires a monthly-cadence candidate); only the rate math was, and it is corrected. Separately noted at the time, the hero card now routes a Fixed-class candidate to no-CTA tip treatment (`:263-269`), so rent no longer leads with "Break it".
...
Status: **Resolved**, Round 2 (current working tree), for the button consolidation only. Habit detail's bespoke `secondaryButton`/`plainButton` styles are removed; the footer actions now render on the shared `Button` (`variant="secondary"` / `variant="tertiary"`), so the vocabulary is back to four button styles. The slate-vs-coral question named in this finding is now resolved at the documentation level (2026-08-22): `constants/theme.ts`'s coral comment previously read "Destructive only (delete, stop breaking, undo import)", contradicting the shipped "stop breaking" trigger, which renders muted slate. The comment now states that "stop breaking" is intentionally slate (a routine choice, not a data loss) and that coral is reserved for hard-destructive actions (delete, undo import), matching the shipped color. No shipped color was flipped; the low-risk doc half of the contradiction is closed.
...
Status: **Resolved (pending Charen's verification)**, 2026-08-22. The code half shipped in Round 2: `ExpenseSheet.tsx` was moved onto the serif `sheetTitle` treatment (`theme.fonts.display` at `typeScale.sheetTitle`), matching AddUpcomingSheet, AddCategoryModal, and CurrencySheet, so the two Money sheets already visibly agree. The documentation half now landed too: `design/PATTERN_VOCABULARY.md` ratifies the serif `sheetTitle` head as THE bottom-sheet pattern under its Surfaces rules ("Bottom sheets head with the serif `sheetTitle` treatment ... AddUpcomingSheet, AddCategoryModal, CurrencySheet, and ExpenseSheet all follow it ... a new sheet reaching for an 11pt eyebrow head instead is a deviation"). Both halves are done; held in the open ledger only until Charen's Lane 2 sign-off confirms the ratification, since a pattern-vocabulary change is a user-facing convention the PR flow routes to a person.
...
Status: **Resolved**, same-day fix work (commit `d5eebfd`). Charen's call resolved in favour of adding the gesture: `components/ui/Sheet.tsx` now implements real swipe-to-dismiss via `PanResponder` driving the single existing `progress` Animated.Value (the one translate driver, native), with a dismiss threshold of 25% of sheet height OR a downward velocity over 0.5 (velocity handoff into the dismiss), a downward-only clamp, spring-back otherwise, a reduced-motion timing fallback, and scroll-safe attachment to the handle zone only. `onAccessibilityEscape` (Round 2, UX-024) is retained. The one-driver-per-node crash rule is preserved: no second driver was added to the translate node. 75 sheet tests pass. The handle now does exactly what it promises.
...
**UX-074 · P2 · DRIFT · layout: the leak-scan results screen's header renders underneath the status bar and dynamic island.**
Location: `app/leak-scan.tsx`, `components/leak-scan/ResultsScreen.tsx`. Not in the original 71; found the same way as UX-073, by running the app in the iOS simulator against seeded demo data.
Impact: neither file references `SafeAreaView` or `useSafeAreaInsets` anywhere (grep-verified), so the results screen's eyebrow ("APR 1 TO JUN 26") and its serif title ("Your statements, read.") render underneath the status bar clock and the dynamic island on a notched device. The screen's own title is partly illegible on the payoff screen of the app's biggest feature.
Fix: wrap the screen in `SafeAreaView`, or apply `useSafeAreaInsets()`'s top inset to the header's padding, matching the pattern already used by every other top-level screen in the app (`paywall.tsx`, `money.tsx`, `(tabs)/index.tsx`, `insights.tsx`, `categories.tsx`, `(tabs)/_layout.tsx`, `onboarding/intent.tsx`, `onboarding/welcome.tsx`, `ui/ScreenHeader.tsx`, `ui/Sheet.tsx` all call `useSafeAreaInsets`). Command: /layout, once the bet picks it up.
Status: **Resolved**, device pass (verified 2026-08-22 against live code). `components/leak-scan/ResultsScreen.tsx` now imports `useSafeAreaInsets` (`:4`), reads it (`:235`), and applies `insets.top + 16` to the header padding (results header `:527`, undo/undone screen `:509`), so the eyebrow and serif title clear the status bar and dynamic island on notched devices. The header no longer renders underneath the notch; the pre-existing gap this finding named is closed.
...
Status: **Resolved**, Round 1 (strings) + device pass (LoggedTodayList; verified 2026-08-22 against live code). `constants/strings.ts:71` stores `keptSoFar: 'Kept so far'` in sentence case (was `'KEPT SO FAR'`), and the second site is now fixed too: `components/money/LoggedTodayList.tsx:104` moves the casing into the stylesheet with `textTransform: 'uppercase'` (with a UX-060 comment noting the string stays sentence case so screen readers speak it as words and the transform stays locale-aware); no JS `.toUpperCase()` remains. Both halves of this finding are closed.
...
Status: **Resolved**, 2026-08-22. `components/onboarding/AuroraBackground.tsx` now carries a code comment above `periodStops` naming the `categoryColors.utility` / `categoryColors.transport` stops as a sanctioned, deliberate decorative borrow (the aurora being one of exactly two decorative gradients the app allows), so the hues read as atmosphere, not category identity, and the borrow is documented rather than silent. Comment only; no color value or logic changed.
...
**UX-072 · P3 · DRIFT · hygiene: category detail rendered raw ISO dates in recent logs.**
Location: `app/category/[id].tsx:190` (the recent-logs row's date `Text` called `item.date.toLocaleDateString()` with no options). Not in the original 71; the fix shipped in commit `377c3b5` on 2026-08-13, but the id was never logged in this document until this pass.
Impact: on Hermes without full ICU, an options-less `toLocaleDateString()` call returns an unformatted, ISO-like string, so every recent log read "2026-08-01 at 8:14 AM" instead of "Aug 1 at 8:14 AM": machine-formatted dates on a screen where every other date is formatted. `formatDate` was already imported in this same file and used a few lines above; this single call had bypassed it. Same class as UX-050, and like UX-073 and UX-074 this was found by running the app with seeded data rather than reading source.
Fix: route through `formatDate(item.date, { month: 'short', day: 'numeric' })`, matching the rest of the file. Command: /clarify.
Status: **Resolved**, commit `377c3b5`. A sweep for other bare `toLocaleDateString` / `toLocaleTimeString` / `toLocaleString` calls across `app/` and `components/` found this was the only site.

**UX-075 · P3 · DRIFT · layout: off-scale micro-radius and off-scale 2pt-spacing literals remain in lower-traffic call sites after the decision 2 scale ratification.**
Location: leftover mechanical cleanup called out in UX-018's own status and in systemic-issue 3: some call sites still hold off-scale micro geometry as literals rather than token references (for example `borderRadius: 3` where `radii.micro` (4) now exists, and 2pt-rhythm spacing literals where the `spacing` scale now exists) even though the values themselves are legitimate under the expanded scale.
Impact: not live drift (the scale ratified these values in Round 1), but literals where a token exists keep drift detection noisy and let the next off-scale value slip in unnoticed.
Fix: convert the remaining off-scale micro-radius (`borderRadius: 3`) and 2pt-spacing literals to `radii.micro` / `spacing` token references; no visual change.
Status: **Resolved**, same-day fix work (commit `d5eebfd`). The conversion pass shipped: every `borderRadius: 3` is now `radii.micro`, and the off-scale padding/margin/gap literals (3/5/7/22) are now `spacing` token references, across 17 files; TierBadge's discs moved to `radii.pill`. One documented KeptHero literal is deliberately retained. tsc is clean and the off-scale grep count is now zero. No pixel changes, token references only.
...
3. **The scales were incomplete, and builders routed around them consistently.** fontSize 14/16/17/26/30, micro radii, and a 2pt spacing rhythm all appeared dozens of times each with no token to reach for. Ratified in Round 1 (decision 2): fontSize literals fell from 116 to 2, and `spacing`/`radii.micro` now exist. The leftover mechanical cleanup this section used to flag (lower-traffic call sites still holding off-scale radius/spacing values as literals) is now closed too: UX-075 converted every `borderRadius: 3` to `radii.micro` and the off-scale spacing literals to `spacing` tokens across 17 files (commit `d5eebfd`), so the off-scale grep count is zero.
...
Status note (updated 2026-08-22): items 1 through 7 below are complete or effectively complete. See the finding-level Status tags and the changelog for exactly what shipped, round by round. What is left is item 8's re-verification pass (the owed on-device VoiceOver/Dynamic Type pass) plus the one remaining open-ledger item: UX-040, resolved in both code and doc and held only for Charen's Lane 2 sign-off. The same-day fix work (commit `d5eebfd`) closed the last engineering and cleanup items, UX-012 (scan yield), UX-041 (swipe-to-dismiss), and UX-075 (token cleanup); UX-060, UX-065, UX-073, and UX-074 were resolved just before it.
...
3. ~~**[P1] `/optimize`**: scan-pipeline yield (UX-012), Spent virtualization (UX-016); decision 3 minimum (UX-019, 057).~~ Done: UX-016, 019, 057 resolved Round 1; UX-012, the scan-pipeline yield, shipped in the same-day fix work (commit `d5eebfd`), so the last unstarted engineering item in this list is now closed.
...
7. **[P2] `/distill`**: vocabulary consolidation (UX-037, 038, 039, 040, 041, 061). Done: UX-041 (swipe-to-dismiss) shipped in the same-day fix work (commit `d5eebfd`); UX-040 is resolved in code and doc (PATTERN_VOCABULARY ratification landed 2026-08-22) and held only for Charen's Lane 2 sign-off.
8. **[P3] `/polish`** as the final pass: everything remaining, plus re-verification. UX-060's residual LoggedTodayList site, UX-065 (AuroraBackground borrow), and the UX-075 token-cleanup pass are all now resolved (2026-08-22, the last in commit `d5eebfd`); what remains is the on-device VoiceOver/Dynamic Type pass this whole report has deferred to Lane 2.
...
- **2026-08-13 (device pass, then id-collision correction)**: two new findings, UX-073 and UX-074, both found by running the app in the iOS simulator against seeded demo data, including a real CSV leak scan, rather than reading source, which is exactly why this audit's earlier rounds missed them: the audit read code, it did not run it. UX-073 (P1, honesty): the leak-scan biggest-leak card's monthly-rate formula (`utils/leakScanBridge.ts:71`) overstated a fixture's true $1,200/month rent rate by roughly 3.3x, rendering "$4,000.00 a month" against an evidence window the same screen states as 86 days; the implied divisor (27 days) was derived arithmetically by solving the formula backwards from the observed output, not confirmed by re-running the pipeline, so that verification is still owed. Rent appearing as a candidate at all is intended behaviour (the eval fixture's own expected output requires it); only the rate math is in question. UX-074 (P2, layout): the leak-scan results screen (`app/leak-scan.tsx`, `components/leak-scan/ResultsScreen.tsx`) has no `SafeAreaView` or safe-area-inset handling anywhere, so its own title renders under the status bar and dynamic island on a notched device; verified pre-existing on `main` (`git show main:app/leak-scan.tsx` and `git show main:components/leak-scan/ResultsScreen.tsx` both show zero safe-area references), not a regression from this branch, whose only touch to that screen's padding was the ratified horizontal 16-to-20 change, vertical untouched.
  - **Id-collision correction, replacing this entry's original numbering note.** The id UX-072 had already been coined, in shipped code and in a commit message (`377c3b5`, "UX-072: category detail showed raw ISO dates in recent logs"), for a real, already-fixed bug: `app/category/[id].tsx:190` called `item.date.toLocaleDateString()` with no options, which on Hermes without full ICU returns an unformatted, ISO-like string instead of a formatted date. But UX-072 had never been logged in this document. When this changelog entry was first written, it correctly found no UX-072 entry here and reused the free id for the leak-scan monthly-rate finding above; given what was visible at the time, that was the right instinct, and inventing an entry to match a code comment it could not verify against this document would have been worse. The actual mistake was coining an id in code and a commit message without ever logging it here. That is now corrected: UX-072 is restored to the meaning it already carries in shipped code and git history, recorded resolved below, and the two findings this entry discovered move to the next free ids, UX-073 (was UX-072 in the first draft of this entry) and UX-074 (was UX-073). Code and git history cannot be rewritten, so the document moved instead; this is the document being corrected to match already-published code.
  - **UX-072 (P3, hygiene), newly logged and already resolved:** category detail rendered raw ISO dates in recent logs. `app/category/[id].tsx:190` called `item.date.toLocaleDateString()` with no options; on Hermes without full ICU that returns an unformatted, ISO-like string, so every recent log read "2026-08-01 at 8:14 AM" instead of "Aug 1 at 8:14 AM". `formatDate` was already imported in the same file and used a few lines above; this one call had bypassed it. Fixed the same day it was found (commit `377c3b5`) by routing through `formatDate(item.date, { month: 'short', day: 'numeric' })`. A sweep for other bare `toLocaleDateString` / `toLocaleTimeString` / `toLocaleString` calls across `app/` and `components/` found this was the only site. Same class as UX-050; like UX-073 and UX-074, found by running the app with seeded data, not by reading source.
  - **None of UX-072, UX-073, or UX-074 moves a dimension score.** None is a regression from the remediation work this audit otherwise tracks, and every dimension score above already carries an explicit caveat that no on-device pass had happened yet. This entry is that overdue device pass turning up findings, plus one earlier fix finally getting logged, not new engineering to weigh into the score; the scores hold exactly where Round 1/2 left them.
  - **Counts**: 71 findings become 74 (UX-072, UX-073, UX-074). Resolved goes from 66 to 67 (the one newly-logged, already-fixed UX-072). Open stays at 7: UX-012, UX-040 (partial), UX-041, UX-060 (partial), UX-065, UX-073, UX-074, the same seven items this document already had open, two of them under new ids. Severity tallies: 2 P0, 18 P1, 32 P2, 22 P3, summing to 74. Both P0s remain resolved. Going forward, ids are unchanged and unrenumbered per the re-run rule; today's correction is the one exception the rule anticipates, made necessary by an id that existed in code before it existed in this document.
  - **The general point, worth naming plainly:** an empty first-run install with no seeded data and no real CSV hid all of this. UX-073's overstated rate needs real multi-month transaction data to compute a wrong number from; UX-074's status-bar overlap needs a real device frame with a notch to render into; UX-072's raw-date bug and UX-050/UX-068's earlier raw-date and timezone bugs all needed real dated rows too. A code read cannot exercise any of these paths; running the app can.

- **2026-08-22 (device-pass close-out + doc/comment closures + new cleanup id)**: three findings the 2026-08-13 device pass surfaced or left half-done are now confirmed fixed in live code, and two more closed via a doc line and a code comment. Nothing here is a score change; it is verification catching up to shipped code plus two documentation closures.
  - **Device-pass fixes now logged resolved after verification against live code (not inferred):** UX-074 (results title under the notch): `components/leak-scan/ResultsScreen.tsx` imports `useSafeAreaInsets` (`:4`), reads it (`:235`), and applies `insets.top + 16` to the header padding (`:527`, and the undo screen at `:509`), so the eyebrow and serif title clear the status bar and dynamic island. UX-073 (biggest-leak monthly-rate overstatement): `ResultsScreen.tsx:250` now computes `rankedLeaksWindowDays = Math.max(result.coverage?.spanDays ?? 0, 1)`, the calendar span rather than the count of transacted days, with an in-code comment citing UX-073; the divisor fix removes the multiples overstatement the rent fixture exposed, and the hero card now routes a Fixed-class candidate to no-CTA tip treatment (`:263-269`). UX-060 (LoggedTodayList casing): `components/money/LoggedTodayList.tsx:104` moves the casing into the stylesheet with `textTransform: 'uppercase'` and a UX-060 comment; no JS `.toUpperCase()` remains, so both halves of this finding (strings.ts in Round 1, LoggedTodayList now) are closed and it is no longer "Partially resolved".
  - **Closed via doc/comment (2026-08-22):** UX-065 (AuroraBackground's borrowed hues): a code comment above `periodStops` in `components/onboarding/AuroraBackground.tsx` now names the `categoryColors.utility` / `categoryColors.transport` gradient stops as a sanctioned, deliberate decorative borrow (the aurora being one of exactly two decorative gradients the app allows), so the borrow is documented rather than silent; comment only, no color or logic changed. UX-040 (Money sheet header pattern): `design/PATTERN_VOCABULARY.md` now ratifies the serif `sheetTitle` head as THE bottom-sheet pattern under Surfaces, matching AddUpcomingSheet, AddCategoryModal, CurrencySheet, and ExpenseSheet; the code half already shipped in Round 2, so both halves are done and the finding is marked resolved-pending Charen's Lane 2 sign-off (a user-facing convention change routes to a person), kept in the open ledger until that sign-off.
  - **The "stop breaking" coral-vs-slate contradiction, closed at the documentation level (low-risk half only):** `constants/theme.ts`'s coral comment previously read "Destructive only (delete, stop breaking, undo import)", which contradicted the shipped "stop breaking" trigger, which renders muted slate. The comment now states that "stop breaking" is intentionally slate (stopping a habit is a routine choice, not a data loss) and that coral is reserved for hard-destructive actions (delete, undo import). No shipped color was flipped; this resolves the documented contradiction in favour of the shipped behaviour, the safe direction (see UX-039).
  - **One new finding, UX-075 (P3, DRIFT, layout), opened in progress:** the leftover off-scale micro-radius (`borderRadius: 3`) and 2pt-spacing literals UX-018's own status already flagged as residual are being converted to `radii.micro` / `spacing` token references, no visual change. Opened as its own id (next free after UX-074) so that mechanical cleanup is tracked and closed explicitly rather than lingering under UX-018.
  - **Counts**: 74 findings become 75 (UX-075). Resolved goes from 67 to 71 (UX-060 completed, UX-073, UX-074, UX-065; UX-040 is resolved-pending-verification and stays in the open ledger). Open goes from 7 to 4: UX-012 (engineering), UX-040 (pending Charen sign-off), UX-041 (decision), UX-075 (in-progress cleanup). Severity tallies: 2 P0, 18 P1, 32 P2, 23 P3, summing to 75. Both P0s remain resolved; every P1 but one (UX-012) is now resolved. Ids are unchanged and unrenumbered per the re-run rule; every resolved finding carries its own Status line.
  - **No score moves.** Every dimension score already carried an explicit "no on-device pass has confirmed this" caveat; this entry is that verification arriving for three findings plus two documentation closures, not new engineering, so the total holds at 19/20 where Round 1/2 left it.

- **2026-08-22 (same-day fix work, commit `d5eebfd`)**: the three findings this document still listed open or in-progress as of the close-out entry above are now resolved in committed code (branch `claude/design-pixel-perfect-spacing-i10zw5`). This is the code catching up to what the audit had already scoped; no new findings, no score change.
  - **UX-012 (P1, resolved)**: the scan pipeline no longer freezes the JS thread. `components/leak-scan/useLeakScanIntake.ts` imports `InteractionManager` and yields via `await new Promise(resolve => InteractionManager.runAfterInteractions(() => resolve()))` immediately before the synchronous `runScan(...)` in `runWithRules`, so the "scanning" stage paints and VoiceOver settles before the blocking parse; the correction re-run path in `components/leak-scan/ResultsScreen.tsx` got the same yield. Regression test at `__tests__/leakScanIntakeYield.test.tsx`. No chunking was added and outputs are unchanged, the freeze is gone because the spinner paints before the parse, not because the parse got faster. This closes the last fully-unstarted engineering item in the report.
  - **UX-041 (P2, resolved)**: the sheet's grab handle now does what it promises. `components/ui/Sheet.tsx` implements real swipe-to-dismiss via `PanResponder` driving the single existing `progress` Animated.Value (the one translate driver, native), with a dismiss threshold of 25% of sheet height OR a downward velocity over 0.5 (velocity handoff into the dismiss), a downward-only clamp, spring-back otherwise, a reduced-motion timing fallback, and scroll-safe attachment to the handle zone only. `onAccessibilityEscape` (Round 2, UX-024) is retained, and the one-driver-per-node crash rule is preserved (no second driver on the translate node). 75 sheet tests pass. The open decision resolved in favour of adding the gesture rather than dropping the handle.
  - **UX-075 (P3, resolved)**: the off-scale radius/spacing token cleanup shipped in full. Every `borderRadius: 3` is now `radii.micro`, and the off-scale padding/margin/gap literals (3/5/7/22) are now `spacing` token references, across 17 files; TierBadge's discs moved to `radii.pill`; one documented KeptHero literal is deliberately retained. tsc is clean and the off-scale grep count is now zero. No pixel changes, token references only.
  - **Counts**: 75 findings, unchanged (no new ids). Resolved goes from 71 to 74; open goes from 4 to 1. The single remaining open item is UX-040, which is resolved in both code and doc (ExpenseSheet on the serif `sheetTitle` head in Round 2, PATTERN_VOCABULARY ratified 2026-08-22) and held in the open ledger only until Charen's Lane 2 sign-off, since ratifying a user-facing convention routes to a person. Severity tallies are unchanged: 2 P0, 18 P1, 32 P2, 23 P3, summing to 75. Both P0s remain resolved; every P1 is now resolved (UX-012, the last open P1, closed here). Ids are unchanged and unrenumbered.
  - **No score moves.** These are engineering and cleanup closures the report had already scoped and priced into the 19/20; the one thing still holding the total short of 20/20 is the owed on-device verification pass, unchanged by this entry.
