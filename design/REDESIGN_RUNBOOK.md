...
- **The log sheet captures a merchant** (Charen, 2026-08-04, overrides spec 04). The prototype's log sheet has no name field; on device that made habit detection unreachable from the app's own logging flow, because `utils/habitDetection.ts` groups strictly on `expense.merchant`. The sheet gains an optional "Where" field plus one-tap chips of recently logged places, and the edit sheet gains the same field. The chips are the mechanism: four logs only accumulate into a leak when the place is spelled the same way each time.
