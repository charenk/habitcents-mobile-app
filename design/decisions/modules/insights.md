...
## Decisions
- 2026-09-10 (QA loop): **"Last 7 days" now sums 7 days, and the Pace card counts the whole last day of the previous month.** Why: the rollup window stepped back 7 days and then floored to midnight, so the card summed 8 calendar days under a 7-day label, and the last-month bound was `new Date(y, m, 0)`, midnight on the final day, which silently dropped everything logged after it. Both windows are pinned by `__tests__/reportsWindows.test.ts` now, and the label reads the same `WEEK_WINDOW_DAYS` the window does, so the two cannot drift again. Visible effect on the returning-user data: the Utilities row leaves Where it went. QA findings 1 and 2.
- 2026-09-10 (Charen): **both empty-state CTAs open the log sheet on Insights.** They used to navigate to Today with `?view=spent&sheet=log`. Same reasoning as [money.md](money.md): the sheet is self-contained on save, and this screen already mounted `PickOneSheet`, so an `ExpenseSheet` beside it is the established shape. The two zero states stay separate in the funnel because each CTA keeps its own `useEmptyStateAction` name (`insights_leaks`, `insights_month`), so the analytics contract is unchanged. Pinned by `__tests__/logInPlace.test.tsx` and `__tests__/insightsFirstScan.test.tsx`.
- 2026-09-07 (Charen): both panes' zero states centre in the pane, the month EmptyState through fill mode and the teaser through the same wrapper geometry by hand. Why: consistent placement of the mark while swiping, app-wide. Residual: the teaser's stack is taller than the month block, so its art sits a little higher; recorded on [LeakFinderTeaser](../components/LeakFinderTeaser.md), not solved here.
- 2026-09-07 (Charen): the Leak finder Zero state's body is one line, down from a 46-word paragraph. Why: it was the heaviest block of text in the app, sitting under the largest illustration, on a pane whose whole job is to read as a light "not yet". The privacy promise stayed, the rest went. See [LeakFinderTeaser](../components/LeakFinderTeaser.md) for what survived and why.
...
## Iterations
- 2026-09-10: Where it went's window narrowed to a true 7 days and the Pace card's last-month window gained its final day; `WEEK_WINDOW_DAYS` is now the one source for both the window and the label.
