...
- Kept Zero: no band, hook (a 96pt money sack since ADR 0036), the three-step explainer (ADR 0039), CTA, and "Break your first habit" in the dock below (ADR 0038). No quote since ADR 0037. Kept Quiet: detection meter n of 4. Kept Live: band then Leaks found / Breaking now; check-in sub-states pending, skipped, slipped, milestone, backfill, weekly. Reach: returning user; milestone, backfill, weekly and Door 3 are canvas only.
- Sheets from Today: log, edit, pick one (and its gate), break habit, partial slip. See [drawers](drawers.md).

## Decisions
- 2026-09-05: both panes' content caps at `layout.contentMaxWidth` (600pt) on tablet widths (`spentScrollContent`, `keptEmptyContent`, `listContent`, the door3 `ribbonWrap`, and a new `keptHeroCapWrap` around `KeptHero`). The pager's paging unit (`pane`, `{ width: screenWidth }`) stays window width by design: `handlePagerMomentumEnd` divides by that same width, so capping the pane itself would break the paging math; only the content inside it caps, same shape as the onboarding carousel's `beat`/`beatContent` split. `keptHeroCapWrap` is a new wrapping View rather than a style spread, because `KeptHero`'s own `card` root already carries a background and a `marginHorizontal` gutter (`keptHeroGutter`); spreading the cap straight onto it would size the background box before the margin insets it, pushing the card past 600pt. routine/ipad.
- 2026-09-05: Kept true-zero carries a three-step explainer under its hook. Why: it is the one state with no data to read, so it is the only place the mechanic can be told. Kept Quiet deliberately does not get one, because by then the detection meter shows real progress against the real threshold. Reverses the one-hook rule for this surface only. ADR 0039.
...
- 2026-09-05: pane content caps at 600pt on tablet widths; `keptHeroCapWrap`/`ribbonWrap` testIDs added; pane itself confirmed to stay window width (routine/ipad).
