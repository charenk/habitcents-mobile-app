...
## Decisions
- 2026-09-07 (Charen): both docks are one structure, [DockCard](../components/DockCard.md): a white pill shell, a snow pill field at a fixed 52pt, a round sage plus. Spent's field holds the amount, Kept's holds the break label. Why: the quick log and the break affordance were two shapes at two heights one swipe apart, so the chrome jumped with the pager. The dashed edge left the Kept dock for the same reason; it stays the in-content "add another" grammar. See [BreakHabitRow](../components/BreakHabitRow.md).
