...
## Decisions
- 2026-09-07 (Charen): the body is one line, down from 46 words. Why: under a 96pt illustration the paragraph read as a wall of text, and the pane sat well outside the app's one-hook standard while claiming to be a light teaser. "Nothing uploads, ever" survived the cut because this pane is the only surface still carrying that promise; the rebuilding-together clause went because the invitation directly below already says "Join the research" and the segment's "Soon" badge already sets the timing.
- 2026-09-07: rejected, moving the detail into a drawer opened by "Count me in". Why: it would have kept the pane clean but made the opt-in two taps, and `drawers.md` records only two sheet families, form and decision. An explainer sheet is a third family and a new decision; cutting words was the cheaper answer to the same problem.
...
## Open
- The teaser centres in its pane (2026-09-07, matching EmptyState's fill mode), but its stack carries a body and a filled button where the month pane's block carries one hook and a text line, so its art sits higher than the month pane's by half the difference. Swiping Insights still shows a small vertical shift. Closing it means either giving the teaser the same block floor with its extra content below the fold of that block, or accepting it as the one pane that is deliberately not an EmptyState. Not solved here.
- **The six months cannot be granted by anything that exists.** `Entitlement` is `'free' | 'premium'` with no duration, start or expiry (`utils/purchases.ts`), stored as one undated string; the only grant path is `setMockEntitlement`, which is dev-menu only and absent from store builds, and `getEntitlement()` is synchronous across eight call sites, so a dated grant has to resolve expiry inside that read. There is no live RevenueCat client and no backend. **A dated grant has to exist before the leak finder ships, or the receipt above becomes a broken promise.** Payments work, so it is Charen's human gate and belongs with the core-p3 branch, not here. On the punch list.
...
- **The two halves of the offer disagree, deliberately for now.** The invitation says "you could win six months" (a draw); the receipt says "Your six months is saved" (a guarantee), matching the 2026-09-06 decision that everyone who opts in gets it. Charen was asked on 2026-09-07 and chose to keep "could win", so this is a known mismatch rather than a bug. A user who opts in reads both lines in sequence, so it is visible from inside the product. Revisit whenever the offer is next touched.

## Iterations
- 2026-09-07: body trimmed to one line.
