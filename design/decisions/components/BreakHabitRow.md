...
## Direction (current)
The Kept pane's dock content: the break-habit affordance, on the same [DockCard](DockCard.md) structure as the Spent pane's quick log, in its own skin: the dashed tone (the app's "add another" edge) and the plain plus (a snow circle with a sage glyph). The label rides in the snow pill field in semibold sage, with the free-plan caption as a second line only at the ceiling, and the round plus on the right is the same control. Both halves open the break sheet, or the paywall at the free ceiling. It is the single re-entry point in both the empty and populated Kept views.
