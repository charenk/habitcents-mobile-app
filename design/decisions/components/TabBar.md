...
- 2026-09-05: labels use `adjustsFontSizeToFit` with a `1/1.5` floor. Why: the "Ins" and "Cate" clipping was horizontal, not vertical, so the taller bar did not fix it; four tabs give each 98pt and "Categories" needs about 91pt at the capped size. The floor means a large-text user never lands below the default 11pt. UX-067, `docs/device-pass.md` section 6.
