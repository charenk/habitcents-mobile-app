...
## Decisions
- 2026-09-07 (Charen, later the same day): content fades into the dock. The scroller above carries a static 36pt [ScrollFade](ScrollFade.md); the dock itself is unchanged and still does not hide, move or measure anything but its own height. A fade that dissolves at the end of the list was considered and deferred: it would be the app's first scroll-driven UI, which the decision below still gates behind an ADR and a release boot walk.
...
## Open
- Closed 2026-09-07: content ending at an invisible line above the pill. Charen saw it on a long log the same day; each pane's scroller now carries a [ScrollFade](ScrollFade.md) at its bottom edge, so the last rows dissolve into the dock's fill rather than cutting off. The fade lives in the scroller's wrapper, not in this dock, so the measured height and the toast lift are untouched.
