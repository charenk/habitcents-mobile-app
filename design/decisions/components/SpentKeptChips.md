...
## Decisions
- 2026-09-12 (localization routine, overflow hardening): the eyebrow label (`spentChipLabel`/`keptChipLabel`) takes `numberOfLines={1}`, matching the amount/placeholder text below it which already had this. Why: the amount and placeholder were already guarded against wrapping (file header, UX-067) but the eyebrow above them was not, so a longer translated eyebrow could wrap and misalign the two segments. No test asserted multi-line before this; no test change needed.
...
## Iterations
- 2026-09-12: `numberOfLines={1}` on both eyebrow labels (overflow hardening).
