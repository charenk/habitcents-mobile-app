...
## Direction (current)
The one pattern for persistent positive communication: sage-light band, 1px sage-tinted border, sprout, one 12.5pt line in sage-dark. Two variants by whether `onDismiss` is passed: dismissible (X) for one-shot receipts, persistent (no X) for standing lines. Placement rule: inside a list section, below the content it comments on, never above an input. `FirstRunRibbon` re-exports it.

## States
Dismissible, persistent. Message and X are separate VoiceOver stops.

## Decisions
- 2026-09-04: generalized from FirstRunRibbon; Door 1's line moves under the log card ahead of the watch nudge; Quiet's line is the persistent variant. Why: above the input it read as an instruction; a dismissible placeholder leaves an empty card. ADR 0033.
