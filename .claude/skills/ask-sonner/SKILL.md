...
<!-- HabitCents local delta (see ../VENDORED.md): disable-model-invocation added.
     Sonner is a React DOM library and cannot run in this React Native app, but
     the app has its own toasts, so without this gate the skill would auto-fire
     on our own toast vocabulary. Kept for reference; invoke by name only. -->
