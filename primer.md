...
- **OPEN P0 BUG (top priority next session):** the app **hard-crashes when tapping the Habits tab on a fresh install** (no data). Not reproduced from reading `app/(tabs)/habits.tsx` / `KeptHero.tsx` / `utils/currency.ts`, so it is almost certainly release/Hermes-only. Next: pull the symbolicated crash log (App Store Connect > TestFlight > Crashes) or reproduce via `npx expo start` / simulator then tap Habits, fix Lane 2, rebuild. This **blocks on-device Tests B (paywall) + C (VoiceOver)**.
