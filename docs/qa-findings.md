...
Real, reproduced, and deliberately deferred with a reason. Not dropped.
...
- [ ] (2026-09-10) [medium] [technical] Leak scan can freeze the JS thread for minutes on legally sized input, with no cancel
  ...
  - Postponed: 2026-09-11 (Charen). Leak scan is out of v1 and stays behind "coming soon" pending a better version, so this is not reachable by a user.
    Revisit when the scan rework starts: the netting refactor, a cell-length cap
    and an async cancellable `runScan` are a design of their own, and the rework
    will likely rewrite that file anyway.
...
- [x] (2026-09-10) [medium] [logical] Categories and category detail double-count an expense that has a custom categoryId and a stored value of "Other"
  ...
  - Fixed: 2026-09-11, PR #162. Shared `resolveExpenseCategory`; all three surfaces resolve to one category before bucketing. Verified on device: Streaming keeps $50.00, Other reads $0.00.
- [x] (2026-09-10) [medium] [telemetry] `coach_moment_shown` FL-1 fires and burns the once-ever flag while the card is never rendered
  ...
  - Fixed: 2026-09-11, PR #164. Fires from the condition that gates the render, matching DT-1. Payload unchanged; the event now means shown rather than eligible (Charen delegated the contract call). One-time repair unspends the flag for pre-fix stores.
- [x] (2026-09-10) [medium] [ux] The onboarding carousel's media frame is left-aligned, not centred, on the first screen a user ever sees
  ...
  - Fixed: 2026-09-11, PR #164. `alignSelf: center` on the frame, not `alignItems` on the beat, which would have re-centred the headline and hook.
- [x] (2026-09-10) [medium] [ux] An unanswered daily check-in sits below the fold behind the leaks section, while the chip's pending dot points at it
  ...
  - Fixed: 2026-09-11, PR #164. Charen reversed the proposed fix: the section order is unchanged, and the dot now means "a leak was found since you last looked at Kept" instead of driving off the check-in.
- [x] (2026-09-10) [low] [technical] A merchant stemming to "constructor" returns a prototype function as its category and display name
  ...
  - Fixed: 2026-09-11, PR #164. `lookupOwn` reads only own properties. `KNOWN_CHAINS` had the same defect and was not in the original report.
- [x] (2026-09-10) [low] [functional] Scientific notation in an amount cell parses as an unrelated number instead of being rejected
  ...
  - Fixed: 2026-09-11, PR #164. Exponents rejected before the character strip; cents derived from the digits of the normalized string rather than a float product.
- [x] (2026-09-10) [low] [ux] The dormant leak-scan redirect lands on "This month", not the Leak finder segment its own contract names
  ...
  - Fixed: 2026-09-11, PR #164. Insights gained a `?view=` param first: the redirect had nothing to pass to, so this was not the one-line change first reported.
- [x] (2026-09-10) [low] [ux] The paywall hand-rolls its buttons outside the six-variant rule, and the Monthly plan row is clipped at rest
  ...
  - Fixed: 2026-09-11, PR #163, layout only. The scroller was unbounded and the footer overlapped it; the Lifetime plan was invisible at rest, which the report missed. The hand-rolled buttons and the `tertiaryBrand` question stay open, recorded on `design/decisions/components/Paywall.md`.
