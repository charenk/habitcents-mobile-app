...
describe('Today: Kept pane chrome tablet cap (routine/ipad item 2e)', () => {
  // The door3 ribbon renders directly in the Kept pane, above and outside
  // the ScrollView/SectionList content that plan item 2b/2d already capped,
  // so it needed its own cap. KeptHero no longer renders in this pane
  // (Charen, 2026-09-07: no band above the list); it now lives only in
  // components/leak-scan/PayoffScreen.tsx. This file already has the
  // provider wiring and fixtures to get the ribbon to render; jest cannot
  // run RN's real flexbox layout engine, so this pins the style contract
  // (same as __tests__/tabletLayout.test.tsx), not a measured pixel width.
