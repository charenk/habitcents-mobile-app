...
>
> **PHASE 2 DESIGN COMPLETE, BUILD PHASE STARTED (2026-07-04).** The design session's full output is landed at `docs/design-package-phase2/` (specs 01-09, interactive prototypes, handoff package). **Every open design question is answered:** Direction C locked (native tokens + motion concentrated on log-save and skip only), taxonomy v2 (10 categories incl. Software & Subscriptions; Transfers/Income/Cash as row classes), dark toggle removed for v1 (revert path documented), onboarding presets and free-tier quiet note adopted. All recorded as **ADRs 0004-0007** in `docs/design-context/decisions/`. **Working mode is now build:** spec-first is satisfied; for each item write nothing new, build exactly to spec (the spec wins over instinct; flag genuine conflicts to Charen). Build order from `docs/design-package-phase2/08-handoff-package.md` section 2: (1) habit-logging surfaces [L], (2) Leak Scan pipeline stages 0-9 + 14 acceptance tests on `task/p2-1b-leak-scan-pipeline` [XL] (1 and 2 parallel), (3) Leak Scan results screen + rule store [L], (4) P2-1 onboarding two-door + Door 1 Leak Audit [L], (5) Coach Moments + remove MICRO_LESSONS [M], (6) P2-4 unification apply pass [L], (7) P2-5 accessibility [M]. Hard rules for every line: leak/skip/kept/slip vocabulary; no em dashes; sentence case; every amount via `useCurrency().format`; green #4CAF50 positive-only, slips neutral; D-9 structural-only analytics; prefers-reduced-motion honored; never commit real bank statements.
>
